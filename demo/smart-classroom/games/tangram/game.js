...
export class TangramGame {
  ...
  container = document.getElementById('canv') || document.querySelector('.game-container') || document.body;
  ...
  constructor() {
    ...
    // 使用默认颜色填充，因为woodTexture.jpeg文件不存在
    this.pattern = '#8B4513'; // 棕色
  ...
  rePositionShapes() {
    ...
    // 确保delta为正数，避免七巧板被移出画布
    const delta = this.canvasWH.map((ele, idx) => {
      const calculatedDelta = ele - maxXY[idx] - this.tL;
      return calculatedDelta > 0 ? calculatedDelta : 20; // 最小偏移量为20
    });
  ...
  renderLoop() {
    ...
    for (let i = 0; i < this.shapes.length; i++) {
      ...
      this.ctx.beginPath();
      this.ctx.moveTo(...shape.vertices[0]);
      ...
      for (let j = 1; j < shape.vertices.length; j++) {
        this.silCtx.lineTo(
          ...shape.vertices[j].map((ele, idx) => (ele - this.centroidTot[idx]) * 100 / this.tL + this.silCanvWH[idx] / 2)
        )
      }
    }
